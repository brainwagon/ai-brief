...
rescoring that followed. The model reading this has changed twice since that
first day, so the calibration records what the Rubric is *for* rather than the
Scores any current model gives it. After editing this file, score a day against
it again and compare, because a sentence that reads well can move a whole Source
by ten Items.
...
- **Small and local.** Models that run on one box, on a GPU that is not a
  ...
  1B parameters is the centre of this, whether it is being released or being put
  to work on somebody's phone, microcontroller, Pi, or laptop CPU. What that is
  worth in Score is in **Scale is not merit** below.
...
- **Adjacent systems and tooling.** Compilers, kernels, GPU programming, file
  formats, databases, developer tools. This Brief is AI-first: the plumbing under
  AI counts, and so does systems work that touches the interests above. Systems
  work with no bearing on any of them is somebody else's Brief.
...
reproducible piece of work squarely in the interests above. Reserve this: the
test is that the Item is worth *acting on*, not merely worth reading. Most Items
that feel like a 5 are a 4 with an exciting title.
...
Real work, real content, and mildly interesting to this reader.
Research whose *finding* would carry outside its own subfield; a model or
dataset that is genuinely new rather than a repackaging; a well-argued article.
**3 is the cutoff — an Item scored 3 appears in the Brief.** Do not give a 3 for
being on-topic. Give it for having something in it.
...
- **Thin input is normal, and length is not quality.** Many Items are a title and
  ...
  abstract about nothing is still a 2. A paper is the exception — judge it by the
  paper rule below, which a title alone can rarely satisfy. Only when the text
  says nothing at all — a repository name with no description — is 2 the honest
  answer for want of evidence.
...
- **If you are hesitating between two Scores, take the lower one.** An Item that
  needs an argument made on its behalf has not earned the higher Score. How many
  Items reach the page is settled after scoring and is not your concern.

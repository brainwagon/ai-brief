...
- **Under 1B parameters is worth one point.** Added on top of what the Item
  earned on its merits, to a language model that size released with weights, or
  to a first-hand account of running one. Give the count and the hardware in the
  Synopsis when it applies. **The point requires a number in the text, under
  1,000M, and three things never take it:** a model of 1B or more, however small
  it sounds beside the rest — 1.5B, 1.7B, 2B, 3B are ordinary Items and score on
  their merits alone; a name, since tiny, mini, nano, and small mean nothing
  without a figure; and a quantisation, GGUF conversion, or re-upload, which
  stays a 2 under the derivative rule no matter how small the original was.

...
- **Scale is not merit.** A bigger model, a bigger cluster, a bigger training run
  ...
  simpler is what earns a point. A language model whose text states a parameter
  count below 1,000M is worth one point on top of its merits, and its Synopsis
  should give the count and the hardware. Nothing else takes that point: not 1B
  or more, so a 1.7B is an ordinary Item; not a name like tiny or nano with no
  figure behind it; and not a quantisation or re-upload, which stays a 2.
